			...
			if ((t.stopPropagation(), $(this).hasClass('selected-nav')))
				...
				($('.content-tint').length = -1) && $('body').append('<div class="content-tint"></div>'),
					a
						...
						.wrap('<div class="blocsapp-special-menu ' + o + '"><blocsnav class="' + l + '">'),
					$('blocsnav').prepend(
				...
				.each(function (t) {
					(fillPadding = 2 * parseInt($(this).css('padding-top'))),
						e.hasClass('bloc-group')
							...
							: (fillBodyHeight = fillBodyHeight + fillPadding + $(this).outerHeight() + 50);
				}),
...
function scrollToTarget(t) {
	1 == t
		...
		? (t = $(document).height())
		: ((t = $(t).offset().top),
		  $('.sticky-nav').length && (t -= $('.sticky-nav .navbar-header').height())),
		$('html,body').animate(
			{
				scrollTop: t,
			},
	...
	window.targetLightbox,
		$(document)
			.on('click', '[data-lightbox]', function (t) {
				...
				targetLightbox.attr('data-gallery-id') && (o = targetLightbox.attr('data-gallery-id')),
					...
					'no-gallery-set' == o
						...
						: (0 == $('a[data-gallery-id="' + o + '"]').index(targetLightbox) &&
						  ...
						  $('a[data-gallery-id="' + o + '"]').index(targetLightbox) ==
								$('a[data-gallery-id="' + o + '"]').length - 1 && $('.next-lightbox').hide()),
